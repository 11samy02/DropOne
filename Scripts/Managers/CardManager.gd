...
## Creates the default deck from the DeckResource definition
...
## Set initial top card to first number card
...
## Update top card, discard pile, and color selection state
...
## Returns true if the played card requires a color selection step
...
## Apply selected wild color and update top card view
...
## Returns the next card in the deck without removing it (Omega peek)
...
## Returns the next N cards from the deck (Omega peek)
...
## Refill deck from discard pile while keeping the current top card
...
## Get current active color (overrides black wilds)
...
## Adds number cards based on number rules
...
## Adds entry cards from the deck entries list
...
## Draws the best card within the next N cards based on a scoring function (Omega)
...
## Inserts a card on top of the deck (debug / cheats)
func force_insert_card_on_top(card: CardResource) -> void:
	if card == null:
		return
	deck.insert(0, card)

## Inserts a card at a specific index in the deck (debug / cheats)
...
## Removes the first card matching a predicate from the deck
